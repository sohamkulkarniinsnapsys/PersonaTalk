...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        // Persist minimal user marker for the legacy dashboard check
        try {
      ...
      <main className="ui-surface-strong w-full max-w-4xl grid md:grid-cols-2 overflow-hidden min-h-[600px] shadow-2xl shadow-black/40">
        ...
        <div className="hidden md:flex flex-col justify-between p-10 relative bg-gradient-to-br from-indigo-500/15 via-slate-900/10 to-slate-950/10">
          <div className="z-10">
            ...
            <h1 className="text-4xl font-bold text-white mb-4 leading-tight">Connect with Intelligence.</h1>
            <p className="text-slate-300 text-lg leading-relaxed">Experience the next generation of video conferencing powered by meaningful AI personas.</p>
          </div>
        ...
        <div className="p-8 sm:p-12 flex flex-col justify-center relative bg-slate-900/30">
          <div className="w-full max-w-sm mx-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="relative group">
                <label htmlFor="password" className="absolute left-4 top-3 text-slate-400 text-sm transition-all pointer-events-none peer-focus:text-xs peer-focus:-translate-y-2 peer-focus:text-indigo-400">
                  Password
                </label>
                <input
